...
This data is stored for as long as the original message exists. It is **automatically deleted** when the original message is deleted on either platform. It is automatically removed if the bridge is later disabled or the bot is removed from the server.
